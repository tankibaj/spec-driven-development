...
**Secondary persona:** Priya — The Operations Manager (see `plan/reference/personas.md`)
...
1. **Cart is client-side only.** Cart lives in browser state (storefront-app). No server-side cart API. Cart contents are sent to the order placement API at checkout time. Cart is not synced across devices — acceptable for anonymous guest shoppers. *(Carried from PDR)*
2. **Products are seeded via API.** Merchants create products by calling `POST /products` directly (via script or fixture). No product management UI in this MVP. *(Carried from PDR)*
3. **notification-service uses MailPit** (local SMTP capture) for MVP. No third-party sign-up or API tokens required. Standard SMTP protocol. *(Carried from PDR)*
4. **Email templates are file-based.** HTML templates bundled with notification-service. No database-backed template management. *(Carried from PDR)*
5. **Admin auth is in-house.** JWT + bcrypt in order-service. TOTP-based MFA for `merchant_owner` and `merchant_admin` roles. No external auth provider. *(Carried from PDR)*
...
- [x] `[BLOCKS APPROVAL]` **Product image support:** The `Product` schema and API do not include an image field. The PDR expects "placeholder thumbnail images" in seed data, and catalogue browsing (AC-001, AC-002) requires product imagery to be a usable catalogue.
...
- [x] `[MINOR]` **Order lifecycle timeline depth:** The PDR mentions "lifecycle timeline" in admin order detail (AC-028). Current contracts store only current `status` and `created_at` — no history of status transitions.
