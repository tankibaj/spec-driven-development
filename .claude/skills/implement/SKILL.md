...
If arguments provided: `$0` = feature-ID, `$1` = slug. Otherwise, list folders in `spec/` and identify the feature with `current_phase: 4`.
...
4. Load `routes.yaml` (root) — map each WP to its target workspace
...
```
FEATURE COMPLETE: {feature-ID}-{slug}
