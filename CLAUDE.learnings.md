...
- 2026-04-11: CLAUDE.md refactored: All phases collapsed to routing logic + skill references (skills own the procedures). Core principle: CLAUDE.md tells the agent WHAT to do and WHICH skill to invoke; skills tell HOW. Avoids redundancy that causes context overload and contradictions between CLAUDE.md and skills.
...
- 2026-04-11: `spec` skill rewritten for minimal interactivity. Old flow: 10 steps with 6 human review gates (AC-by-AC approval). New flow: 4 steps — resolve PRD open questions (only if needed), then generate IA + FS autonomously in one pass. FS includes Assumptions and Open Questions with options + agent recommendations. Human reviews files directly and updates `status.yaml`. Rationale: the old flow burned too much context and time; the PRD already captures intent, so the agent has enough to generate a complete draft.
- 2026-04-11: `plan` skill updated with pre-flight validation gate. Before generating TS + WPs, the skill checks: all artifacts approved, no unresolved `[BLOCKS APPROVAL]` open questions, every IA service has ≥1 AC, ACs consistent with resolved open questions, referenced contracts exist. If pre-flight fails, generation does not start — the human fixes the FS/IA first. This catches spec bugs before they become expensive TS/WP bugs.
...
- 2026-04-11: `implement` skill created for Phase 4. Key design decisions: (1) one skill with BE/FE reference files, not two separate skills; (2) orchestrates ALL WPs for a feature, launching parallel agents for independent WPs targeting different workspaces; (3) low-interaction — WP is the approved spec, agent only stops for pre-flight failure, contract conflicts, 3 failed fix attempts, or genuinely unresolvable ambiguity; (4) standard checkpoint taxonomy (BE: scaffold→models→routes→integration→tests→dod; FE: scaffold→state→components→integration→tests→dod); (5) explicit resume protocol for interrupted sessions (check git log, re-run tests, continue from last_checkpoint); (6) wave failure is dependency-graph-aware — blocked WP holds only dependent downstream WPs, unaffected WPs continue; (7) each parallel agent writes only its own phase_4.WP-XXX entry in status.yaml.
- 2026-04-11: Branching strategy agent instructions moved from `contracts/architecture/branching-strategy.md` to `.claude/rules/branching-strategy.md`. Agent now auto-creates branches following naming conventions — never asks the human. The contracts doc retains human-facing content (naming, merge strategy, branch protection).
- 2026-04-11: Contract freeze rule (`.claude/rules/contracts-readonly-phase4.md`) updated: on contract conflict, agent must provide a recommendation (what's wrong, additive vs breaking, affected consumers, suggested fix), not just STOP and block.
